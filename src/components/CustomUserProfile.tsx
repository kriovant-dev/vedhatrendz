...
const CustomUserProfile: React.FC<CustomUserProfileProps> = ({ className = '' }) => {
  ...
  const getInitials = (user: any): string => {
    ...
    if (user.primaryPhoneNumber?.phoneNumber) {
      return user.primaryPhoneNumber.phoneNumber.slice(-2);
    }
    return 'U';
  };
  ...
  const getPhoneNumber = (user: any): string => {
    return user.primaryPhoneNumber?.phoneNumber || 'Not provided';
  };

  // Calculate days since joined
  const getDaysSinceJoined = (createdAt: string | number | Date): number => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center text-sm text-muted-foreground mt-1">
                ...
                <span className="truncate">{getPhoneNumber(user)}</span>
              </div>
              <div className="flex items-center text-xs text-muted-foreground mt-1">
                <Calendar className="h-3 w-3 mr-1" />
                <span>
                  Member for {getDaysSinceJoined(user.createdAt)} days
                </span>
